...
select concat(first_name, ' ', last_name) from employees.employees where last_name like 'e%';
...
select datediff(now(), hire_date) from employees.employees where hire_date like
'199%' and birth_date like'%-12-25'order by birth_date asc, hire_date desc;
...
select first_name, count(first_name) from employees.employees where first_name not like '%a%' group by first_name limit 10;